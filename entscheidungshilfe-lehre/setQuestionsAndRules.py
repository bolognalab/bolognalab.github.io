...
for code in list_of_scenarios:
    ...
    # favor syncInt=2
        ...
        if code.split("-")[3] == "3":
            updated_questions["intSync"]["antworten"]["1"]["effects"][code] = -2
        # exclude fully asynchronous options
        if code.split("-")[3] == "4":
            updated_questions["intSync"]["antworten"]["1"]["effects"][code] = -100

    # If max synchronous interaction is just in plenum (LZsy = 2)
    # - exlude syncInt=0 or 2
    if code.split("-")[2] in ["0", "2"]:
        updated_questions["intSync"]["antworten"]["1"]["effects"][code] = -100
    #favor syncInt=1 :
    else:
        updated_questions["intSync"]["antworten"]["1"]["effects"][code] = +1
        # - discourage options with asynchronous participation alternative
        if code.split("-")[3] == "3":
            updated_questions["intSync"]["antworten"]["1"]["effects"][code] = -1
        # - discourage fully asynchornous options
        elif code.split("-")[3] == "4":
            updated_questions["intSync"]["antworten"]["1"]["effects"][code] = -3

    # If teaching style ist mostly didactic
    # exclude syncInt=2, favor syncInt=1
    if code.split("-")[2] == "2":
        updated_questions["intSync"]["antworten"]["0"]["effects"][code] = -100
    ...
    if lehrform in ["hyb", "onl", "ringonl", "ringhyb2", "onlhybwechs", "hybrem"]:
    ...
    if lehrform in ["onlpraeswechs", "hybpraeswechs"]:  
    ...
    # TODO: add effects for special conditions etc.
    ...
    #TODO: add special condition if course is hybrid (maybe not in this file...)
